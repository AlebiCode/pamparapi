...
public class UiManager : MonoBehaviour
{
    ...
    private IEnumerator UiInitialization()
    {
        ...
        yield return null;  //a quanto pare non tutte le ui sono sistemate dal canvas nel primo frame. Quindi aspetto un frame. Magari non è nemmeno più così lol
        ...
        //questo se il canvas scaler è settato a conservare la larghezza!
    ...
    private void CrossWordUi()
    {
        ...
        crossWordWindow_offScreenPosition = new Vector2(0, crossWordWindow.rect.height);
    ...
    {
        StartCoroutine(AnimateCrosswordPanelsCoroutine(onScreen));
    }

    private IEnumerator AnimateCrosswordPanelsCoroutine(bool onScreen)
    {
        float animationTime = 0.2f;
        if (onScreen)
        {
            RandomEmoticons.instance.RestartEmotions();

            fullTastieraPanel.DOAnchorPos(tastiera_onScreenPosition, animationTime);
            
            crossWordWindow.localScale = Vector3.zero;
            crossWordWindow.anchoredPosition = crossWordWindow_onScreenPosition;
            crossWordWindow.DOScale(Vector3.one, animationTime).SetEase(Ease.OutExpo);
        }
        else
        {
            RandomEmoticons.instance.StopEmotions();

            fullTastieraPanel.DOAnchorPos(tastiera_offScreenPosition, animationTime);
            
            crossWordWindow.DOScale(Vector3.zero, animationTime).SetEase(Ease.InOutExpo);
            yield return new WaitForSeconds(animationTime);
            crossWordWindow.anchoredPosition = crossWordWindow_offScreenPosition;
            crossWordWindow.localScale = Vector3.one;
